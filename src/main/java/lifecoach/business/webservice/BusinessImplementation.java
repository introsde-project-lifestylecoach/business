...
public class BusinessImplementation implements Business
{
	...
	public List<GoalBusiness> getGoals(int pId) {
		...
		for(Goal g: gList)
		{
			...
			gbList.add(gb);
			if (g.getGoalType().getType().equals("increase")) {
			...
			} else if (g.getGoalType().getType().equals("decrease")) {
			...
			} else if (g.getGoalType().getType().equals("daily")) {
				System.out.println("Check goal for daily");
				// TODO

			} else if (g.getGoalType().getType().equals("amount")) {
